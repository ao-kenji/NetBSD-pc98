/*	$NecBSD: init.c,v 1.5 1998/02/08 08:00:34 kmatsuda Exp $	*/
/*	$NetBSD$	*/
...
int mode;
{
	...
	while (dev->name) {
		...
		if ((boot = read_boot()) == NULL)
			goto try_again;
#ifdef PC98 /* for HUMAN68K floppy */
		if ( !bcmp( (char *)(boot)+2, "Hudson soft", 11) ) {
			/* HUMAN DISK. It has no disk descriptor */
			boot->secsiz[0] = 0x00;
			boot->secsiz[1] = 0x04;
			boot->clsiz = 0x01;
			boot->nrsvsect[0] = 0x01;
			boot->nrsvsect[1] = 0x00;
			boot->nfat = 2;
			boot->dirents[0] = 0xc0;
			boot->dirents[1] = 0x00;
			boot->psect[0] = 0xd0;
		...
		heads = WORD(nheads);
		sectors = WORD(nsect);
#ifdef RELYTABLE /* Mainly support for HDD formatted with exform */
                if (heads != dev->heads) {
                    /* tracks will be calculated afterwards. */
                    heads	= dev->heads;
                    sectors	= dev->sectors;
                }
#endif /* RELYTABLE */
		...
		if (!heads || heads > 100 || !sectors || sectors > 500 || tracks > 5000 || !boot->clsiz) {
			...
			unsigned char temp[MSECTOR_SIZE_MAX];
#else
...
read_boot()
{
	...
	if (lseek(fd, (off_t)disk_offset, 0) < (off_t)0)
		return(NULL);
