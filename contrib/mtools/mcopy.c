/*	$NecBSD: mcopy.c,v 1.5 1998/02/08 08:00:41 kmatsuda Exp $	*/
/*	$NetBSD$	*/
...
/* Enable '-z' option of mwrite command. By Hirose Yuuji 1992/1/13 */
#include <stdio.h>
#include <ctype.h>
#include <sys/types.h>
#include <sys/wait.h>
...
char *argv[];
{
	...
	oops = 0;
#ifdef SMARTNAME
#define MWOPTIONS "tnvmzp"
#else
#define MWOPTIONS "tnvmz"
#endif /* SMARTNAME */
	while ((i = getopt(argc, argv, MWOPTIONS)) != EOF) {
		switch (i) {
			case 't':
			case 'n':
			case 'v':
	...
	if (oops || (argc - optind) < 2) {
		...
		fprintf(stderr, "Usage: %s [-tnvmz] sourcefile targetfile\n", argv[0]);
		fprintf(stderr, "       %s [-tnvmz] sourcefile [sourcefiles...] targetdirectory\n", argv[0]);
		exit(1);
	}
