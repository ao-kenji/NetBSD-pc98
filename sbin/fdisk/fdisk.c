/*	$NecBSD: fdisk.c,v 1.25.2.2 1999/08/20 04:49:04 kmatsuda Exp $	*/
...
#ifndef	ORIGINAL_CODE
/*
 * XXX: for comatibility...
 *	sys/disklabel_mbr.h	machine/disklabel.h
 */
#define	mbrp_flag		dp_flags
#define	mbrp_shd		dp_shd
#define	mbrp_ssect		dp_ssect
#define	mbrp_scyl		dp_scyl
#define	mbrp_typ		dp_typ
#define	mbrp_ehd		dp_ehd
#define	mbrp_esect		dp_esect
#define	mbrp_ecyl		dp_ecyl
#define	mbrp_start		dp_start
#define	mbrp_size		dp_size
#endif	/* PC-98 */
#define LBUF 100
...
#else	/* PC-98 */
struct mboot {
	u_int8_t bootinst[DOSPARTOFF - sizeof(u_int16_t)];
	u_int16_t	signature;	
	struct	dos_partition parts[NDOSPART];
	u_int8_t mb_padding[DEV_BSIZE];
...
#endif
#else	/* PC-98 */
#define OPTIONS			"01234567Safius:b:c:"
#endif	/* PC-98 */

#define ACTIVE 0x80
#ifndef	ORIGINAL_CODE
#define BOOT_MAGIC 0xAA55
#define MBR_MAGIC	BOOT_MAGIC

#define MBR_PSECT(s)	(s)
#define MBR_PCYL(c,s)	(c)
#endif	/* PC-98 */
...
unsigned char bootcode[] = {
	,
};
#endif	/* PC-98 */

...
#else	/* PC-98 */
void	change_part __P((int, sysid_t, int, int));
#endif	/* PC-98 */
...
#else	/* PC-98 */
void	dos __P((int, unsigned short *, unsigned char *, unsigned char *));
#endif	/* PC-98 */
...
#else	/* PC-98 */
	int cstart, csize;	/* For the b_flag. */
	sysid_t csysid;
#endif	/* PC-98 */
...
#else	/* PC-98 */
		init_sector0(dos_cylindersectors, 1);
#endif	/* PC-98 */
...
#else	/* PC-98 */
		init_sector0(dos_cylindersectors, 0);
#endif	/* PC-98 */
...
#else	/* PC-98 */
	(void)fprintf(stderr, "usage: fdisk [-aiufSc] [-0|-1|...|-7] "
#endif	/* PC-98 */
		      "[device]\n");
	exit(1);
}
...
#else	/* PC-98 */
	empty = (dos_partition_read_sysid(partp) == (sysid_t) 0);
#endif	/* PC-98 */

#ifndef	ORIGINAL_CODE
	sysid = dos_partition_read_sysid(partp);
	start = disk_start_sector(&disklabel, partp);
	size = disk_size_sector(&disklabel, partp);
#endif	/* PC-98 */
...
#else	/* PC-98 */
	printf("<sysid/sid/mid> 0x%lx/0x%lx/0x%lx (%s)\n", 
		sysid, sysid_to_sid(sysid), sysid_to_mid(sysid),
		get_type(sysid_to_mid(sysid)));
	printf("    start %ld, size %ld (%ld MB), flag 0x%x\n",
	    start, size, size * 512 / (1024 * 1024), 0);
#endif	/* PC-98 */
	...
	int start, dopart;
{
	...
	u_long dos_disksectors = dos_cylinders * dos_heads * dos_sectors;
	...
	/* XXX: make a partition 0 as full disk size */
	partp = &mboot.parts[0];
	strcpy(partp->dp_name, "NetBSD/pc98");
	dos_partition_write_sysid(partp, SYSID_NETBSD);
	dos(start, &partp->dp_scyl, &partp->dp_shd, &partp->dp_ssect);
	dos(start, &partp->dp_ipl_cyl, &partp->dp_ipl_head,
		&partp->dp_ipl_sct);
	dos(dos_disksectors - dos_cylindersectors, &partp->dp_ecyl,
		&partp->dp_ehd, &partp->dp_esect);
#endif	/* PC-98 */

	if (!sh_flag)
		printf ("MBR partition table initialized.\n");
}
...
#else	/* PC-98 */
	/* 
	 * The ONLY kernel knows the disk geometry. Never guess!
	 */
#endif	/* PC-98 */
}
...
#else	/* PC-98 */
		dos_partition_write_sysid(partp, csysid);
		/* XXX: shuold be fixed! */
		if (is_sysid_bsd((sysid_t) csysid) != 0)
		{
			memset(partp->dp_name, 0, sizeof(partp->dp_name));
			strncpy(partp->dp_name, "NetBSD/pc98", 16);
		}
		dos(cstart,
		    &partp->dp_scyl, &partp->dp_shd, &partp->dp_ssect);
		dos(cstart,
		    &partp->dp_ipl_cyl, &partp->dp_ipl_head,
		    &partp->dp_ipl_sct);
		dos(cstart + csize - dos_cylindersectors,
...
#else	/* PC-98 */
			int sid, mid;	/* for user interfaces XXX */

			csysid = dos_partition_read_sysid(partp);
			cstart = disk_start_sector(&disklabel, partp);
			csize = disk_size_sector(&disklabel, partp);
			sid = sysid_to_sid(csysid);
			mid = sysid_to_mid(csysid);
			decimal("sid", &sid);
			decimal("mid", &mid);
			decimal("start", &cstart);
			decimal("size", &csize);

			csysid = sysid_make_id(mid, sid);
			dos_partition_write_sysid(partp, csysid);
#endif	/* PC-98 */
		}
...
#else	/* PC-98 */
			if (is_dos_partition_active(partp + part))
#endif	/* PC-98 */
				active = part;
	}
	if (!f_flag) {
...
#else	/* PC-98 */
	for (part = 0; part < NMBRPART; part++)
		dos_partition_deactivate(partp + part);
	if (active < NMBRPART)
		dos_partition_activate(partp + active);
#endif	/* PC-98 */
}
...
#ifndef	ORIGINAL_CODE
		disklabel.d_ncylinders = b_cyl;
		disklabel.d_ntracks = b_head;
		disklabel.d_nsectors = b_sec;
#endif	/* PC-98 */
		return;
	}
...
#else	/* PC-98 */
	unsigned short *cylinderp;
	unsigned char *headp, *sectorp;
#endif	/* PC-98 */
{
	int cylinder, head;
	...
	*cylinderp = cylinder;
	if (head != 0 || sector != 0)
	{
		printf("CRITICAL WARNING: head and sector should be ZERO!!\n");
	}
	*headp = head;
	*sectorp = sector;
#endif	/* PC-98 */
...
#ifndef	ORIGINAL_CODE
	if (read_disk(1, (u_int8_t *) mboot.parts) == -1) {
		warn("can't read fdisk partition table");
		return (-1);
	}
#endif	/* PC-98 */
...
write_s0()
{
	...
	if (ioctl(fd, DIOCWLABEL, &flag) < 0)
		warn("DIOCWLABEL");
#ifndef	ORIGINAL_CODE
	if (write_disk(1, (u_int8_t *) mboot.parts) == -1) {
